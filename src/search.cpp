...
Move Search::search(Position *pos, TimeManager *manager, int id) {
    ...
    for (Depth d = 1; d <= depth; d++) {
        ...
        score = alphabeta(&threadPos, sd, alpha, beta, d);

        if (infoStrings && id == 0 && !tm->forceStop)
...
inline Value eval(Position *pos) {
    Value eval = 0;

    Color side = pos->side();

    for (PieceType pt = PAWN; pt < KING; ++pt) {
        Bitboard us = pos->pieces(side, pt);
        Bitboard them = pos->pieces(~side, pt);

        Value val = piece_value(pt);
        eval += val * popcount(us);
        eval -= val * popcount(them);
    }
    return eval;
}
...
Value Search::alphabeta(Position *pos, SearchData *sd, Value alpha, Value beta, Depth depth) {
    ...
    // Reset pv table length
    sd->pvTable[sd->ply].reset();

    if (depth <= 0 || depth >= MAX_PLY || sd->ply >= MAX_PLY)
        return eval(pos);
    ...
    if (sd->threadId == 0 && sd->nodes % 1024 == 0) {
        if (tm->elapsed() > tm->moveTimeLimit.max) {
            tm->stop();
            return beta;
        }
    }
    ...
    if (sd->ply && pos->is_draw()) {
        return (sd->ply >= MAX_PLY && !pos->checks()) ? eval(pos) : 8 - (sd->nodes & 0xF);
    }
    ...
    while ((m = gen.next_best<LEGAL>()) != Move::none()) {
        ...
        if (infoStrings && sd->ply == 0 && sd->threadId == 0 && tm->elapsed() > 3000)
            std::cout << "info depth " << sd->rootDepth << " currmove "
                      << from_move(m, false) << " currmovenumber " << moveCnt << std::endl;

        pos->do_move<true>(m);
        sd->ply++;

        score = -alphabeta(pos, sd, -beta, -alpha, depth - 1);
    ...
    if (legalMoves == 0) bestScore = pos->checks() ? mated_in(sd->ply) : VALUE_DRAW;
